...
```bash
pip install chromadb google-genai opencv-python-headless openai python-dotenv streamlit yt-dlp streamlit
```

3. Provide your Gemini / OPENAI API key in an environment variable or `.env` file:
...
- The default vision provider is Gemini (`gemini-2.5-flash`)/ OpenAI (GPT-5) and default embedding model is `gemini-embedding-001`.
